...
⚠️ **Required**: You must provide your mod ID using `ksp.arg("lapis.modId", ...)`
...
## `@Patcher` — Add State & Logic to Existing Classes
...
Unlike `@Unlocker`, `@Patcher` doesn't just expose internals—it **adds new Kotlin logic directly to target instances** using Mixins, bridges, and auto-generated extension APIs.
...
## `@Unlocker` — Open Private & Protected Members with Clean Kotlin
